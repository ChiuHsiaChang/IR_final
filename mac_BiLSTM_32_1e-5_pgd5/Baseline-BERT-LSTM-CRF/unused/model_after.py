...
class CRFModel(BaseModel):
    def __init__(self,
        ...
        self.classifier = nn.Linear(out_dims, num_labels)
    ...
    def forward(self,
                ...
                labels=None,
                pseudo=None):
        token_ids, input_token_starts = input_data
        ...
        origin_sequence_output = [layer[starts.nonzero().squeeze(1)]
                                  for layer, starts in zip(seq_out, input_token_starts)]
        ...
        if labels is not None:
            if pseudo is not None:
                # (batch,)
                tokens_loss = -1. * self.crf(emissions=emissions,
                                                    tags=labels.long(),
                                                    mask = labels.gt(-1),
                                                    reduction='none')
        ...
        # else:
        #     tokens_out = self.crf.decode(emissions=emissions, mask=attention_mask.byte())
...
class EnsembleCRFModel:
    def __init__(self, model_path_list, bert_dir_list, num_labels, device, lamb=1/3):
        ...
        for idx, _path in enumerate(model_path_list):
            ...
            model = CRFModel(bert_dir=bert_dir_list[0], num_labels=num_labels)
            ...
            if idx == 0:
                ...
                self.crf.load_state_dict(model.crf.state_dict())
                self.crf.to(device)
    ...
    def predict(self, model_inputs):
        ...
        tokens_out = self.crf.decode(emissions=logits, mask=attention_mask.byte())
...
class EnsembleSpanModel:
    def __init__(self, model_path_list, bert_dir_list, num_labels, device):
        ...
        for idx, _path in enumerate(model_path_list):
            ...
            print(f'Load model type: {bert_dir_list[0]}')
            model = SpanModel(bert_dir=bert_dir_list[0], num_labels=num_labels)
